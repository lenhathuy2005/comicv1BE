...
async function getCultivationSummary(userId) {
  const rows = await query(
    ...
    SELECT
      ...
      l.afk_exp_per_min,
      l.afk_gold_per_min,
      l.is_breakthrough_level,
      l.breakthrough_to_realm_id,
      nl.exp_required AS next_level_exp_required,
      CONCAT(COALESCE(r.name, 'Cảnh giới'), ' tầng ', COALESCE(l.stage_number, l.level_number, 1)) AS level_name,
      r.id AS realm_id,
      r.realm_order,
...
async function getMyProfile(userId) {
  // Chuẩn hóa tiến độ cảnh giới trước khi trả profile.
  // Tầng 1-9 đủ EXP sẽ tự lên tầng; tầng 10 giữ đầy EXP để chờ đột phá.
  await cultivationService.normalizeCultivationProgress(userId);
  ...
  return {
    ...
    cultivation_summary: cultivation
      ? {
          ...
          stage_number: cultivation.stage_number != null ? Number(cultivation.stage_number) : null,
          stage_band: cultivation.stage_band || null,
          stage_name: cultivation.stage_name || null,
          realm_id: cultivation.realm_id,
          realm_order: cultivation.realm_order != null ? Number(cultivation.realm_order) : null,
          ...
          afk_multiplier: Number(cultivation.afk_multiplier || 1),
          afk_exp_per_min: Number(cultivation.afk_exp_per_min || 0),
          afk_gold_per_min: Number(cultivation.afk_gold_per_min || 0),
          is_breakthrough_level: Boolean(cultivation.is_breakthrough_level),
        }
      : null,
    vip_summary: vip
      ? {
