...
async function listCultivationAdmin() {
  const [realms, levels, breakthroughRules, snapshots] = await Promise.all([
    ...
    query(`
      SELECT br.*, fr.name AS from_realm_name, tr.name AS to_realm_name,
             req.name AS required_item_name, ins.name AS insurance_item_name
      ...
      ORDER BY br.breakthrough_level ASC
    `),
  ...
  return {
    overview: {
      ...
      totalBreakthroughRules: breakthroughRules.length,
      finalRealm: realms[realms.length - 1]?.name || null,
      finalLevel: levels[levels.length - 1]?.level_number || null,
    },
    realms,
    levels,
    breakthroughRules,
